...
<button class="feedbackButton" onclick="openFeedbackModal()" aria-label="<?php echo esc_attr($lwsoptimize_t['buttonText']) ?>">
    <span class="feedbackButton__text"><?php echo esc_html($lwsoptimize_t['buttonText']) ?></span>
</button>
...
<div class="modalOverlay" id="feedbackModal" style="display: none;">
    <div class="modalContent">
        <div class="modalHeader">
            <h3><?php echo esc_html($lwsoptimize_t['modalTitle']) ?></h3>
        ...
        <div id="feedbackForm" class="form">
            <form onsubmit="handleSubmit(event)">
                <div class="formGroup">
                    <label for="feedbackType"><?php echo esc_html($lwsoptimize_t['typeLabel']) ?></label>
                    <select id="feedbackType" name="type" required>
                        <?php foreach ($lwsoptimize_t['types'] as $lwsoptimize_key => $lwsoptimize_value): ?>
                            <option value="<?php echo esc_attr($lwsoptimize_key) ?>"><?php echo esc_html($lwsoptimize_value) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <div class="formGroup">
                    <label for="feedbackName"><?php echo esc_html($lwsoptimize_t['nameLabel']) ?></label>
                    <input type="text" id="feedbackName" name="name" placeholder="<?php echo esc_attr($lwsoptimize_t['namePlaceholder']) ?>">
                </div>

                <div class="formGroup">
                    <label for="feedbackEmail"><?php echo esc_html($lwsoptimize_t['emailLabel']) ?></label>
                    <input type="email" id="feedbackEmail" name="email" placeholder="<?php echo esc_attr($lwsoptimize_t['emailPlaceholder']) ?>">
                </div>

                <div class="formGroup">
                    <label for="feedbackMessage"><?php echo esc_html($lwsoptimize_t['feedbackLabel']) ?></label>
                    <textarea id="feedbackMessage" name="feedback" placeholder="<?php echo esc_attr($lwsoptimize_t['feedbackPlaceholder']) ?>" rows="4" required></textarea>
                </div>

                <div class="formActions">
                    <button type="button" onclick="closeFeedbackModal()" class="cancelButton">
                        <?php echo esc_html($lwsoptimize_t['cancelButton']) ?>
                    </button>
                    <button type="submit" class="submitButton" id="submitBtn">
                        <?php echo esc_html($lwsoptimize_t['submitButton']) ?>
                    </button>
                </div>
            </form>
        </div>
...
<script>
const translations = <?php echo wp_json_encode($lwsoptimize_t) ?>;
