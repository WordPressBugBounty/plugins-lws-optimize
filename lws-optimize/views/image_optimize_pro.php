        ...
        if (conversion_listing && jQuery.fn.DataTable) {
            ...
            if (data['images_listing'] && Object.keys(data['images_listing']).length > 0) {
                Object.values(data['images_listing']).forEach(function(image) {
                    ...
                    let is_converted = image['converted'] && !image['unavailable'];
                    let link_url = (is_converted && image['converted_url']) ? image['converted_url'] : image['url'];
                    let full_path = (is_converted && image['converted_path']) ? image['converted_path'] : image['path'];
                    let name_cell = `${image['name']}`;
                    if (link_url) {
                        name_cell += ` <a href="${escapeHtml(link_url)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(full_path || '')}" class="lwsop_image_link">
                            <img src="<?php echo esc_url(plugins_url('images/lien.svg', __DIR__)); ?>" alt="<?php esc_attr_e('Open image', 'lws-optimize'); ?>" width="14px" height="13px">
                        </a>`;
                    }
                    if (image['converted']) {
