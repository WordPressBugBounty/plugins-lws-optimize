...
class LWSCache
{
    ...
    private function load_dependencies()
    {
        ...
        if (!class_exists("LWSCache_Loader")) {
            require_once plugin_dir_path(dirname(__FILE__)) . 'cache/class-lws-cache-loader.php';
        }
        ...
        if (!class_exists("Purger")) {
            require_once plugin_dir_path(dirname(__FILE__)) . 'cache/class-purger.php';
        }
        ...
        if (!class_exists("LWSCache_Admin")) {
             ...
             */
            require_once plugin_dir_path(dirname(__FILE__)) . 'cache/class-lws-cache-admin.php';
        }
    ...
    private function define_admin_hooks()
    {
        ...
        if (! empty($lws_cache_admin->options['cache_method']) && 'enable_redis' === $lws_cache_admin->options['cache_method']) {
            if (class_exists('Redis')) { // Use PHP5-Redis extension if installed.
                ...
                if (class_exists('Predis_Purger')) {
                    require_once plugin_dir_path(dirname(__FILE__)) . 'cache/class-predis-purger.php';
                }
            ...
            if (!class_exists("FastCGI_Purger")) {
                require_once plugin_dir_path(dirname(__FILE__)) . 'cache/class-fastcgi-purger.php';
            }
        ...
        if (is_multisite()) {
            ...
            $this->loader->add_filter('network_admin_plugin_action_links_' . LWS_OP_BASENAME, $lws_cache_admin, 'lws_cache_settings_link');
        } else {
            $this->loader->add_action('admin_menu', $lws_cache_admin, 'lws_cache_admin_menu');
            $this->loader->add_filter('plugin_action_links_' . LWS_OP_BASENAME, $lws_cache_admin, 'lws_cache_settings_link');
        }
