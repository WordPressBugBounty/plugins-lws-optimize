...
class WpFastestCacheLazyLoad
{
            ...
            switch ($GLOBALS["wp_fastest_cache_options"]->wpFastestCacheLazyLoad_placeholder) {
                case "default":
                    if (isset($GLOBALS["wp_fastest_cache"]->content_url) && $GLOBALS["wp_fastest_cache"]->content_url) {
                        $this->placeholder = WPFC_PLUGIN_URL . "/pro/images/blank.gif";
                    } else if (defined('WPFC_WP_CONTENT_URL')) {
                        $this->placeholder = preg_replace("/https?\:\/\//", "//", WPFC_PLUGIN_URL) . "/pro/images/blank.gif";
                    } else {
                        $this->placeholder = WPFC_PLUGIN_URL . "/pro/images/blank.gif";
                    }
                    break;
                ...
                case "base64":
                    ...
                    break;
                default:
                    $this->placeholder = preg_replace("/^custom_/", "", $GLOBALS["wp_fastest_cache_options"]->wpFastestCacheLazyLoad_placeholder);
            }
        } else {
            $this->placeholder = preg_replace("/https?\:\/\//", "//", WPFC_PLUGIN_URL) . "/pro/images/blank.gif";
        }
    ...
    public function video_to_lazyload($data, $inline_scripts)
    {
        ...
        if (!empty($video_list)) {
            foreach (array_reverse($video_list) as $key => $value) {
                ...
                if ($this->is_exclude($video_html)) {
                    continue;
                }

                if (!preg_match("/<source[^\>]+src\s*\=\s*[^\>]+>/i", $video_html)) {
                    // <video class="elementor-background-video-hosted elementor-html5-video" autoplay muted playsinline loop></video>
                    continue;
                }


                $video_html = "<noscript data-type=\"wpfc\">" . $video_html . "</noscript>";
            ...
            foreach ($matches[0] as $key => $img) {
                ...
                if (!preg_match("/" . preg_quote($img, "/") . "/i", $inline_scripts)) {
                    ...
                    if ($this->is_exclude($img)) {
                        ...
                        $tmp_img = preg_replace("/\sloading\=[\"\']lazy[\"\']\s*/i", " ", $img);
                        ...
                        if ($key < $offset) {
                            ...
                            if (preg_match("/\ssrc\s*\=[\"\'][^\"\']+[\"\']/i", $img)) {
                                if (preg_match("/mc\.yandex\.ru\/watch/i", $img)) {
                                    ...
                                    $tmp_img = preg_replace("/\ssizes\s*\=/i", " data-wpfc-original-sizes=", $tmp_img);
    ...
    public function iframe_to_lazyload($content, $inline_scripts)
    {
        ...
        if (count($matches[0]) > 0) {
            foreach ($matches[0] as $iframe) {
                ...
                // <iframe data-placeholder-image="/wp-content/plugins/complianz-gdpr/assets/images/placeholders/google-maps-minimal-1280x920.jpg" data-cmplz-target="src" data-src-cmplz="https://maps.google.com/maps?q=Paseo%20Pedro%20Sim%C3%B3n%20A" data-deferlazy="1" src="about:blank"></iframe>
                if (preg_match("/data-src-cmplz/", $iframe)) {
                    continue;
                }
                ...
                if (!preg_match("/" . preg_quote($iframe, "/") . "/i", $inline_scripts)) {
                    if (!preg_match("/onload=[\"\']/i", $iframe)) {
                        ...
                        if (preg_match("/(youtube|youtube-nocookie)\.com\/embed/i", $iframe) && !preg_match("/videoseries\?list/i", $iframe)) {
                            ...
                            $tmp_iframe = preg_replace("/\ssrc\=[\"\'](https?\:)?\/\/(www\.)?(youtube|youtube-nocookie)\.com\/embed\/([^\"\']+)[\"\']/i", " onload=\"Wpfcll.r(this,true);\" data-wpfc-original-src=\"" . WPFC_PLUGIN_URL . "/pro/templates/youtube.html#$4\"", $iframe);
                        } else {
    ...
    {
        $js = "\n<script data-wpfc-render=\"false\">" . file_get_contents(WPFC_PLUGIN_PATH . "/pro/js/lazy-load-new.js") . "</script>\n";
    ...
    {
        $js = "\n<script data-wpfc-render=\"false\">" . file_get_contents(WPFC_PLUGIN_PATH . "/pro/js/lazy-load.js") . "</script>\n";
